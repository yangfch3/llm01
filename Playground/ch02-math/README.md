...
| 脚本 | 内容 |
|---|---|
| `vector_matrix.py` | 手写点积、矩阵乘 vs NumPy 内置；形状错误演示 |
| `softmax_cross_entropy.py` | 数值稳定 softmax + CE；解析梯度 vs 数值梯度 |
| `gradient_chain_rule.py` | 两层网络解析反向 vs 数值梯度对照 |
| `mlp_numpy.py` | 纯 NumPy 两层 MLP，过拟合二维双高斯团（ch03 PyTorch 版的对照） |
